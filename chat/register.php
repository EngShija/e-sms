	...
	if(isset($_FILES['file2']['name'])&&$_POST['Change'])	{
				 ...
				 if($id=='')
				 {
                       ...
                       $useremail=$user['email'];
                          ...
                          $retrieved = mysqli_query(database(),$sqluser);
 	 ...
 	 $qued="SELECT * FROM Profilepictures WHERE ids='$id' && Category='$cat' ";
                     $resul=mysqli_query(database(),$qued);
...
if(isset($_POST['postid'])){
	...
	if(isset($_POST['replied'])) 
        {$texts=addslashes($_POST['replied']);
      		$text = mysqli_real_escape_string(database(),$texts);
		}
		 if(isset($_POST['replied']))
    	     {  
				$fname = $chatUsers['Firstname'];
        	    //   $sql = "SELECT * FROM Users WHERE Password='$userid'";
	            //    $resultset = mysqli_query(database(), $sql) or die("database error:". mysqli_error(database()));
	            //       while($row = mysqli_fetch_assoc($resultset)){ $fname= $row['Firstname'];}	
                    ...
                    database()->query($query1z) or die('Error, query failed');
              
			}
	   
	$get="SELECT * FROM Chart WHERE Group_Name='' ";
						    $gets=mysqli_query(database(),$get);						   
      ...
      while($get_row=mysqli_fetch_array($gets))
      {
		...
		//$day=$get_row['Today'];
                  ...
                  $times=date("g:i a",$time);
				        ...
				        //    $sqluserk ="SELECT * FROM Users WHERE Email ='$useremail'";
                            // $ret = mysqli_query(database(),$sqluserk);
...
if(isset($_POST['loadid'])){
 ...
 if(isset($_POST['gnamed'])){               
		  ...
		  $sql="SELECT * FROM Groups  WHERE GName='$gname'";
                   ...
                   $resultnL=mysqli_query(database(),$sqlL);                    
      		   ...
      		   $text = mysqli_real_escape_string(database(),$texts);
        	      ...
        	      $sql = "SELECT * FROM Users WHERE Password='$userid'";
	               $resultset = mysqli_query(database(), $sql) or die("database error:". mysqli_error(database()));
                    ...
                    "VALUES ('$text','$fname','$userid','database()Format','$date','$postgroup')";
                    database()->query($query1z) or die('Error, query failed');
